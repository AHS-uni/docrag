...
from docrag.schema.raw import SlideVQARawEntry
from docrag.schema.utils import tag_inferred, tag_missing, tag_low_quality
...
class SlideVQAUnifier(Unifier[SlideVQARawEntry]):
        ...
        if count_pages == 0:
            if self.test_mode:
                ...
                self._problematic["documents"].append({
                    "document_id": raw.deck_name,
                    "reason": "Missing in corpus."
                })
                return None
            else:
                raise UnificationError(f"Document '{raw.deck_name}' not found in corpus.")

        document = Document(id=raw.deck_name, count_pages=count_pages)
        document.tags.append(tag_missing("type", "SlideVQA does not provide document type."))
        return document
    ...
    def _build_question(self, raw: SlideVQARawEntry) -> Question:
        ...
        if raw.answer_type or raw.resoning_type:
            ...
            question.tags.append(tag_inferred("type", "Inferred using answer_type and resoning_type."))
            return question
    ...
    def _build_evidence(self, raw: SlideVQARawEntry) -> Evidence:
        ...
        if missing:
            if self.test_mode:
                ...
                self.logger.warning(
                    ...
                    missing, raw.deck_name, evidence.pages,
                )
        ...
        if raw.answer_type:
            ...
            if sources:
                ...
                evidence.tags.append(tag_low_quality("sources", "Evidence source information is limited in SlideVQA. Only provides distinction between span and non-span evidence source."))
            else:
                evidence.tags.append(tag_missing(
                    "sources",
    ...
    def _build_answer(self, raw: SlideVQARawEntry) -> Answer:
        ...
        if raw.arithmetic_expression and raw.arithmetic_expression.lower() != "none":
            ...
            answer.tags.append(tag_missing("rationale", "No rationale provided for this answer."))
    ...
    def _map_evidence_source(self, answer_type: str | None) -> list[EvidenceSource]:
        if answer_type and answer_type.lower().startswith(("single-span", "multi-span")):
