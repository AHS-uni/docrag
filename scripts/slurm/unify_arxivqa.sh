...
from docrag.unify import ArxivQAUnifier
from docrag.datasets import load_corpus_dataset, load_qa_dataset, push_dataset_to_hub

TOKEN = os.environ["TOKEN"]
ROOT = Path("data/arxivqa")

unifier = ArxivQAUnifier(name="ArxivQA", data_dir=ROOT)
unifier.unify()

corpus_ds = load_corpus_dataset(ROOT, cast_image=True, streaming=False)
push_dataset_to_hub(corpus_ds, repo_id="AHS-uni/arxivqa-corpus", token=TOKEN)

# 3) Load & push QA splits
splits = {
    "train": "unified_qas/arxivqa.jsonl",
}
qa_ds = load_qa_dataset(ROOT, splits=splits, include_images=False, streaming=False)
push_dataset_to_hub(qa_ds, repo_id="AHS-uni/arxivqa-qa", token=TOKEN)
PYCODE
